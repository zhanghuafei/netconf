...
public class NetworkBindingDomWriteCandidateTx implements NetworkWriteTransaction{ 
  ...
  private DOMDataWriteTransaction getOrCreate(InstanceIdentifier<?> mountPointPath,
    ...
    if (tx == null) {
      ...
      DOMDataBroker db = mountPoint.getService(DOMDataBroker.class).get(); // I think not check
  ...
  public CheckedFuture<Void, TransactionCommitFailedException> submit() {
    ...
    final ListenableFuture<Void> commitFutureAsVoid = Futures.transform(netTxStatus, new Function<RpcResult<TransactionStatus>, Void>() {
      ...
      public Void apply(final RpcResult<TransactionStatus> input) {
          Preconditions.checkArgument(input.isSuccessful() && input.getErrors().isEmpty(), "Submit of net transaction failed with error: %s", input.getErrors());
          return null;
      }
  });
